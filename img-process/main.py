...
def main():
    ...
    # 创建目录
    if not os.path.exists(line_seg_dir):
        os.makedirs(line_seg_dir)
    ...
    with open(line_seg_dir + '/lines.txt', 'w') as f:
        for i, original_name in enumerate(original_names):
            if original_name.split('.')[-1] != 'png':  # 抛出系统隐藏文件异常
                continue
            # 以图片名创建目录用于存储每个segmentation
            if not os.path.exists(line_seg_dir + '/' + original_name.split('.')[0]):
                os.makedirs(line_seg_dir + '/' + original_name.split('.')[0])
            # 获取图片
            ...
            if image_type == classify.image_type1:  # 虚线
                ...
                coordinate_cells = segment_img.table_out(table_fit_image)  # 获取单元格坐标
                h_threshold = 66
            ...
            for j, cell in enumerate(coordinate_cells):
                ...
                cell_name = original_name.split('.')[0] + '-' + '{:0>3d}'.format(name_index) + '.' + original_name.split('.')[-1]
                if segment_img.has_text(rotated_image[y1:y2, x1:x2], image_type) is True:  # 单元格内有文本
                    if segment_img.has_multiple_text(cell[-1], h_threshold) is False:  # 单行文本
                        y_extend = 8  # 向下多取8像素
                        h_extend = cell[3]+y_extend if cell[1]+cell[3]+y_extend<=rotated_image.shape[0] else rotated_image.shape[0]-cell[1]  # 三元表达式防止越界
                        cell[3] = h_extend
                        """写入txt"""
                        f.write(cell_name + ' ' + str(tuple(cell)) + '\r\n')  #格式: 文件名 + 坐标
                        cell_image = rotated_image[y1:(y1 + h_extend), x1:x2]
                        cell_image_filename = line_seg_dir + '/' + original_name.split('.')[0] + '/' + cell_name
                        """保存图片"""
                        ...
                        if (cell_image.shape[0]>1000 and cell_image.shape[1]>200) or (cell_image.shape[1]>600 and cell_image.shape[0]>100):
                            if 197 < cell_image.shape[0] < 500 and cell_image.shape[1] > 1000:  # 表头
                                ...
                                cell_image = segment_line.rm_vertical_line(cell_image)
                                cell_image = segment_line.rm_horizontal_line(cell_image)
                                cell_image = segment_line.rm_noise(cell_image)
                                line_ctrs = segment_line.header_segment(cell_img_cpy, cell_image)
                                for line in line_ctrs:
                                    line_name=original_name.split('.')[0]+'-'+'{:0>3d}'.format(name_index)+'.'+original_name.split('.')[-1]
                                    ...
                                    line_image_filename = line_seg_dir + '/' + original_name.split('.')[0] + '/' + line_name
                                ...
                                cell_image = segment_line.rm_horizontal_line(cell_image)
                                cell_image = segment_line.rm_noise(cell_image)
                                line_text_imgs, line_text_ctrs = segment_line.line_segment(cell_img_cpy, cell_image)
                                ...
                                for ltc in line_text_ctrs:
                                    ...
                                    ltc[1] = ltc[1] + y1
                                    """写入txt"""
                                    temporary_line_name=original_name.split('.')[0]+'-'+'{:0>3d}'.format(temporary_index)+'.'+original_name.split('.')[-1]
                            ...
                            cell_image = segment_line.rm_horizontal_line(cell_image)
                            cell_image = segment_line.rm_noise(cell_image)
                            line_ctrs = segment_line.num_img_segment(cell_img_cpy, cell_image, area_th)
                            for line in line_ctrs:
                                line_name=original_name.split('.')[0]+'-'+'{:0>3d}'.format(name_index)+'.'+original_name.split('.')[-1]
                                y1_l, y2_l, x1_l, x2_l = line[1], line[1] + line[3], line[0], line[0] + line[2]
                                # 将分割图坐标更新为在整张图上的坐标
